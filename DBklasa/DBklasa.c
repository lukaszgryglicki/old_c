...
int lsprintf(char* str, char* fmt, ...)
{
 ...
 nfmt = (char*)malloc(strlen(fmt)+40);
 j=0;
...
int getDistinctStudent(int* id, PGconn* connection)
{
 ...
 if (i<0) { printf("Brak klas, wiec brak tez uczniow.\n"); return 1; }
 ...
 if (!pgres)
   {
    printf("Blad wykonania zapytania SQL, tablica uczen nie istnieje?\n");
    printf("%d\n", __LINE__);
    return 1;
   }
 ...
 printf("Dostepni studenci:\n");
 for (i=0;i<a;i++) printf("%-30s\t %-25s\t %s\n", out[i][1],out[i][0],out[i][3]);
 ln();
 free_p3c(&out,a,b);
 printf("\nPodaj nazwisko studenta (lub jego czesc): ");
 takestring(nazwisko);
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak takiego nazwiska: %s!\n",nazwisko);
    return 1;
   }
 if (a>1)
   {
    free_p3c(&out,a,b);
    printf("Wybor niejednoznaczny, nastepujacy studenci spelniaja wymagania:\n");
    for (i=0;i<a;i++) printf("%-30s\t %-25s\t %s\n", out[i][1],out[i][0],out[i][3]);
    ln();
    printf("Podaj imie studenta (lub jego czesc): ");
    takestring(imie);
    ...
    if (!a)
      {
       free_p3c(&out,a,b);
       printf("Brak takiego studenta: %s %s!\n",nazwisko,imie);
       return 1;
      }
    if (a>1)
      {
       ...
       for (i=0;i<a;i++) printf("%-30s\t %-25s\t %s\n", out[i][1],out[i][0],out[i][3]);
       ln();
       printf("Musisz podac tez klase bo wybor imienia i nazwiska jest niejednoznaczny...\n");
       displayClasses(connection);
       printf("Nazwa klasy (lub fragment nazwy): ");
       shortstring(klasa, 8);
       ...
       if (!pgres)
         {
          printf("Blad wykonania zapytania SQL, tablica klasa nie istnieje?\n");
    printf("%d\n", __LINE__);
          return 1;
         }
       out = pgres2pc3(pgres, &a, &b);
       ...
       if (!a)
         {
          free_p3c(&out,a,b);
          printf("Nie ma takiej klasy!\n");
          return 1;
         }
      if (a>1)
         {
          free_p3c(&out,a,b);
          printf("Wybor nie jest jednoznaczny, %d klas spelnia wymagania!\n",a);
          for (i=0;i<a;i++) printf("%s, ", out[i][0]);
          ln();
          return 1;
         }
       idklasy = atoi(out[0][1]);
       if (idklasy<0) { free_p3c(&out,a,b) ; printf("Blad wyznaczania idklasy!\n"); return 1; }
       printf("Wybrana klasa: %s\n", out[0][0]);
       free_p3c(&out,a,b);
...
int addStudentsToClass(PGconn* connection)
{
 ...
 if (!a)
   {
    printf("Dodajemy pierwszego ucznia...\n");
   }
 printf("Dostepni studenci:\n");
 for (i=0;i<a;i++) printf("%-30s\t %-25s\t %s\n", out[i][1],out[i][0],out[i][3]);
 ln();
 free_p3c(&out,a,b);
 printf("Wybierz klase do ktorej uczeszcza uczen:");
 i=displayClasses(connection);
 if (i<=0) { printf("\nBrak zdefiniowanych klas!\n"); return 1; }
 printf("Nazwa klasy (lub fragment nazwy): ");
 shortstring(klasa, 8);
 check = (char*)malloc(300);
 if (!check) { ERR printf("malloc failed!\n"); return 1; }
 lsprintf(check,"SELECT nazwa,idklasy from klasa WHERE nazwa LIKE '|'", klasa);
 pgres = execute_query(connection, check);
 free(check);
 ...
 while (1)
  {
   printf("Imie (wpisz stop aby zakonczyc): ");
   takestring(imie);
   ...
   if (a)
     {
      free_p3c(&out,a,b);
      printf("Taki student: %s,%s juz istnieje w tej klasie!\n",imie,nazwisko);
      free(userCmd);
      return 1;
     }
   free_p3c(&out,a,b);
   ...
   if (!pgres)
     {
      printf("Blad przy dodawaniu ucznia.\n");
    printf("%d\n", __LINE__);
      free(userCmd);
      userCmd = 0;
      return 1;
...
int getDistinctClass(int* idklasy, PGconn* connection)
{
 ...
 if (!pgres)
   {
    printf("Blad wykonania zapytania SQL, tablica przedmiot nie istnieje?\n");
    printf("%d\n", __LINE__);
    return 1;
   }
 out = pgres2pc3(pgres, &a, &b);
 ...
 *idklasy = atoi(out[0][1]);
 free_p3c(&out,a,b);
 if (*idklasy<0) { printf("Blad wyznaczania idklasy!\n"); return 1; }
 return 0;
}
...
int getDistinctSubject(int* idprzedmiotu, PGconn* connection)
{
 ...
 if (i<=0) { printf("\nBrak zdefiniowanych przedmiotow!\n"); return 1; }
 printf("Nazwa przedmiotu (lub fragment nazwy): ");
 ...
 if (a>1)
   {
    free_p3c(&out,a,b);
    printf("Wybor nie jest jednoznaczny, %d przedmiotow spelnia wymagania!\n",a);
    for (i=0;i<a;i++) printf("%s, ", out[i][0]);
    ln();
    return 1;
   }
 *idprzedmiotu = atoi(out[0][1]);
 free_p3c(&out,a,b);
 if (*idprzedmiotu<0) { printf("Blad wyznaczania idprzedmiotu!\n"); return 1; }
 return 0;
}
...
int removeMark(char** userCmd, PGconn* connection)
{
 ...
 if (!pgres)
   {
    printf("Blad wykonania zapytania SQL, tablica ocena nie istnieje?\n");
    printf("%d\n", __LINE__);
    return 1;
   }
 out = pgres2pc3(pgres, &a, &b);
 ...
 if (!a)
   {
    printf("Brak wstawionych ocen.\n");
    return 1;
   }
 free_p3c(&out,a,b);
 printf("Wybierz ucznia (wpisz nieistniejacego aby przerwac):\n");
 i=getDistinctStudent(&iducznia,connection);
 if (i) { printf("Blad przy wyborze studenta.\n"); return 1; }
 printf("Wybierz przedmiot:\n");
 i=getDistinctSubject(&idprzedmiotu,connection);
 if (i) { printf("Blad przy wyborze przedmiotu.\n"); return 1; }
 ...
 *userCmd = 0;
 ...
 if (!a)
   {
    printf("Ten uczen nie ma oceny z tego przedmiotu.\n");
    return 1;
   }
 if (a>1)
   {
    free_p3c(&out,a,b);
    printf("Ten uczen ma wiecej niz 1 ocene, to oznacza powazny blad!\n");
    ERR
    free(*userCmd);
    *userCmd = 0;
    return 1;
...
int getSubjectByID(PGconn* connection, int idprzedmiotu, char* nazwa)
{
 ...
 if (!a)
   {
    printf("Brak takiego przedmiotu\n");
    return 1;
   }
 strcpy(nazwa,out[0][0]);
 free_p3c(&out,a,b);
 ...
 *userCmd = 0;
 ...
 for (i=0;i<a;i++)
 {
  ...
  if (err) { printf("Blad w wyznaczaniu ID ucznia\n"); return 1; }
  err=getSubjectByID(connection,atoi(out[i][3]),przedmiot);
  if (err) { printf("Blad w wyznaczaniu ID przedmiotu\n"); return 1; }
 ...
 *userCmd = 0;
 ...
 if (i) { printf("Blad w wyznaczaniu ID ucznia\n"); return 1; }
 i=getSubjectByID(connection,idprzedmiotu,przedmiot);
 if (i) { printf("Blad w wyznaczaniu ID przedmiotu\n"); return 1; }
 if (!strcmp(licz,"f")) strcpy(licz,"nie");
 else strcpy(licz,"tak");
 printf("Ocena ucznia %s %s z klasy %s z przedmiotu %s to %s, liczona w sredniej: %s\n",nazwisko,imie,klasa,przedmiot,ocena,licz);
 return 0;
}
...
int seeMarksBySubject(char** userCmd, PGconn* connection)
{
 ...
 free_p3c(&out,a,b);
 *userCmd = (char*)malloc(512);
 printf("Wybierz przedmiot (nieistniejacy aby przerwac)\n");
 i=getDistinctSubject(&idprzedmiotu,connection);
 if (i) { printf("Blad przy wyborze przedmiotu.\n"); return 1; }
...
int modifyMark(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    printf("Brak ocen do modyfikacji.\n");
    return 1;
   }
 free_p3c(&out,a,b);
 printf("Wybierz ucznia (wpisz nieistniejacego aby przerwac):\n");
 ...
 free_p3c(&out,a,b);
 if (stara<0.0) { printf("Blad wyznaczania oceny do modyfikacji.\n"); return 1; }
 printf("Podaj nowa ocene zamiast %f,liczona:%s (podaj 0 aby anulowac): ",stara,czl);
 shortstring(tmp, 15);
 wartosc = -1.0;
 sscanf(tmp, "%f", &wartosc);
 if (wartosc<1.0 || wartosc>6.0) { printf("To nie jest poprawna ocena.\n"); return 1; }
 czlicz:
 printf("\nCzy ocena ma byc liczona w sredniej? (t/n): ");
 czyliczona = mgetch();
 ln();
 if (czyliczona>='A' && czyliczona<='Z') czyliczona+=0x20;
 if (czyliczona!='n' && czyliczona!='t') { printf("Bledna odpowiedz!\n"); goto czlicz; }
 *userCmd = (char*)malloc(300);
 if (!*userCmd) { ERR printf("malloc failed!\n"); return 1; }
...
int addMarkByClass(char** userCmd, PGconn* connection)
{
 ...
 out = pgres2pc3(pgres, &a, &b);
 clear_result(&pgres);
 if (!a) printf("Dodajemy pierwsza ocene...\n");
 free_p3c(&out,a,b);
 i=getDistinctClass(&idklasy,connection);
 if (i) { printf("Blad przy wyborze klasy.\n"); return 1; }
 i=getClassByID(connection, idklasy, klasa);
 if (i) { printf("Blad przy wyborze klasy.\n"); return 1; }
 printf("Dodajemy studentow do klasy %s\n", klasa);
 check = (char*)malloc(strlen(klasa)+200);
 if (!check) { ERR printf("malloc failed!\n"); return 1; }
 ...
 for (i=0;i<a;i++)
   {
    ...
    if (err) { printf("Blad w wyznaczaniu nazwy przedmiotu!\n"); free_p3c(&out,a,b); return 1; }
    printf("Przedmiot: %s, liczony(t), nie(n), bez ocen(b), koniec(k): (t/n/b/k) ",przedmiot);
    err = mgetch();
    ln();
    ...
    if (!ia)
      {
       free_p3c(&iout,ia,ib);
       free_p3c(&out,a,b);
       printf("Brak uczniow w tej  klasie!\n");
       return 1;
      }
    for (j=0;j<ia;j++)
      {
       ...
       if (err) { printf("Blad w wyznaczaniu danych studenta!\n"); free_p3c(&iout,ia,ib); free_p3c(&out,a,b); return 1; }
       ...
       if (iia)
         {
          ...
          printf("Uczen: %s %s %s z %s to:\n", nazwisko,imie,klasa,przedmiot);
          printf("Ocena tego ucznia z tego przedmiotu jest juz wstawiona!\n");
	  printf("Nacisnij k aby zakonczyc wstawianie\n");
	  printf("Nacisnij p aby przejsc do nastepnego przedmiotu\n");
	  printf("Wstawic nowa? (t/n/k/p) ");
          err = mgetch();
          ln();
          if (err>='A' && err<='Z') err+=0x20;
	  if (err=='k')
	  ...
	  if (err!='t') continue;
	   ...
	   if (!pgres)
	     {
              printf("Blad w usuwaniu oceny, nie mozna jej zmienic!\nSprobuj (m)odyfikowac ocene!\n");
	      continue;
	     }
         }
       ocenan:
       printf("Ocena %s %s %s z %s to:\n", nazwisko,imie,klasa,przedmiot);
       printf("1.0-6.0\tnormalna ocena\n");
       printf("0\tnie wstawiaj tej oceny\n");
       printf("7\tnastepny przedmiot\n");
       printf("8\trezygnuj z wstawiania ocen\n");
       printf(">>>\t");
       scanf("%s", tmp);
       wartosc=-1.0;
       sscanf(tmp,"%f", &wartosc);
       ...
       if (wartosc<1.0 || wartosc>6.0) { printf("Bledna ocena!\n"); goto ocenan; }
       *userCmd = (char*)malloc(300);
       if (!*userCmd) { ERR printf("malloc failed!\n"); return 1; }
       ...
       if (!pgres)
         {
          printf("Blad przy dodawaniu oceny.\n");
          printf("%d\n", __LINE__);
          free(*userCmd);
          *userCmd = 0;
          return 1;
...
int addMarkByStudent(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    printf("Dodajemy pierwsza ocene...\n");
   }
 free_p3c(&out,a,b);
 printf("Wybierz ucznia (wpisz nieistniejacego aby przerwac):\n");
 i=getDistinctStudent(&iducznia,connection);
 ...
 free_p3c(&out,a,b);
 printf("Podaj ocene (podaj 0 aby anulowac): ");
 shortstring(tmp, 15);
 wartosc = -1.0;
 sscanf(tmp, "%f", &wartosc);
 if (wartosc<1.0 || wartosc>6.0) { printf("To nie jest poprawna ocena.\n"); return 1; }
...
int addMarkBySubject(char** userCmd, PGconn* connection)
{
 ...
 free_p3c(&out,a,b);
 printf("Podaj ocene (0 aby anulowac): ");
 shortstring(tmp, 15);
 wartosc = -1.0;
 sscanf(tmp, "%f", &wartosc);
 if (wartosc<1.0 || wartosc>6.0) { printf("To nie jest poprawna ocena.\n"); return 1; }
...
int addMark(char** userCmd, PGconn* connection)
{
 ...
 idprzedmiotu = atoi(out[0][1]);
 free_p3c(&out,a,b);
 if (idprzedmiotu<0) { printf("Blad wyznaczania idklasy!\n"); return 1; }
 i=getDistinctStudent(&iducznia,connection);
 if (i) { printf("Blad przy wyborze studenta.\n"); return 1; }
 pgres=0;
 out=0;
...
int addSubject(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    printf("Dodajemy pierwszy przedmiot\n");
   }
 printf("Dodajac nowy przedmiot nie uzywaj nazwy juz istniejacych:\n");
 for (i=0;i<a;i++) printf("%s, ",out[i][0]);
 printf("\n");
 free_p3c(&out,a,b);
 printf("Nazwa przedmiotu (max 30 liter): ");
 takestring(tmp);
 len = strlen(tmp);
 if (len>30) { printf("Nazwa jest dluzsza niz 30!\n"); return 1; }
 *userCmd = (char*)malloc(len+100);
 if (!*userCmd) { ERR printf("malloc failed!\n"); return 1; }
...
int addClass(char** userCmd, PGconn* connection)
{
 ...
 out = pgres2pc3(pgres, &a, &b);
 clear_result(&pgres);
 if (!a) printf("Dodajemy pierwsza klase.\n");
 printf("Dodajac nowa klase nie uzywaj nazwy juz istniejacych:\n");
 for (i=0;i<a;i++) printf("%s, ",out[i][0]);
 printf("\n");
 free_p3c(&out,a,b);
 printf("Nazwa klasy (max 7 liter): ");
 takestring(tmp);
 len = strlen(tmp);
 if (len>7) { printf("Nazwa jest dluzsza niz 7!\n"); return 1; }
 *userCmd = (char*)malloc(len+100);
 if (!*userCmd) { ERR printf("malloc failed!\n"); return 1; }
 sprintf(*userCmd,"INSERT INTO klasa VALUES ('%s')", tmp);
 pgres=0;
 out=0;
 check = (char*)malloc(150);
 if (!check) { printf("check malloc failed!\n"); return 1; }
 ...
 if (a)
   {
    free_p3c(&out,a,b);
    printf("Taka klasa juz istnieje!\n");
    return 1;
   }
 free_p3c(&out,a,b);
 return 0;
...
int seeStudents(char** userCmd, PGconn* connection)
{
 ...
 if (!pgres)
    {
     printf("Blad wykonania zapytania SQL, tablica klasa lub uczen nie istnieje?\n");
    printf("%d\n", __LINE__);
     return 1;
    }
 out = pgres2pc3(pgres, &a, &b);
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak uczniow spelniajacych kryteria!\n");
    return 1;
   }
...
void seeSubjects(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak zdefiniowanych przedmiotow!\n");
    return;
   }
 printf("Oto zdefioniowane przedmioty:\n");
 for (i=0;i<a;i++) printf("przedmiot %-30s\t ID systemowe %s\n",out[i][0],out[i][1]);
 printf("\n");
 free_p3c(&out,a,b);
}
...
void seeClasses(char** userCmd, PGconn* connection)
{
 ...
 printf("Oto zdefioniowane klasy:\n");
 for (i=0;i<a;i++) printf("klasa %-10s\t ID systemowe %s\n",out[i][0],out[i][1]);
 printf("\n");
 free_p3c(&out,a,b);
}
...
int modifyStudentIndirect(PGconn* connection, char* userCmd,char* nazwisko, char* imie, int idklasy)
{
 ...
 strcpy(nimie,imie);
 nidklasy=idklasy;
 printf("Modyfikowac nazwisko? (t/n): ");
 z = mgetch();
 ln();
 if (z>='A' && z<='Z') z+=0x20;
 if (z=='t') n=1;
 printf("Modyfikowac imie? (t/n): ");
 z = mgetch();
 ln();
 if (z>='A' && z<='Z') z+=0x20;
 if (z=='t') i=1;
 printf("Modyfikowac klase? (t/n): ");
 z = mgetch();
 ln();
 if (z>='A' && z<='Z') z+=0x20;
 if (z=='t') k=1;
 ...
 if (k)
   {
    printf("Oto dostepne klasy:\n");
    z=displayClasses(connection);
    if (z<0) { printf("Brak zdefiniowanych klas!\n"); return 1; }
    printf("Podaj nazwe klasy (lub jej czesc): ");
    shortstring(tmp,8);
    ...
    if (nidklasy<0) { free_p3c(&out,a,b) ; printf("Blad wyznaczania idklasy!\n"); return 1; }
    printf("Wybrana klasa: %s\n", out[0][0]);
    free_p3c(&out,a,b);
   }
 check = (char*)malloc(250);
 ...
 if (a)
      {
       free_p3c(&out,a,b);
       printf("Taki student juz istnieje!\n");
       strcpy(userCmd,"");
       return 1;
      }
 free_p3c(&out,a,b);
...
int modifyIndexStudent(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak studentow do modyfikacji!\n");
    return 1;
   }
 printf("Dostepni studenci:\n");
 ...
 if (iducznia<0) { printf("Bledne ID!\n"); return 1; }
 check = (char*)malloc(270);
 *userCmd = (char*)malloc(300);
 if (!*userCmd || !check) { ERR printf("malloc failed!\n"); return 1; }
 ...
 if (err) { printf("Blad modyfikacji ucznia!\n"); return 1; }
 return 0;
}
...
int removeStudent(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak studentow do usuniecia!\n");
    return 1;
   }
 printf("Dostepni studenci:\n");
 for (i=0;i<a;i++) printf("%-30s\t %-25s\t %s\n", out[i][1],out[i][0],out[i][3]);
 ...
 *userCmd = (char*)malloc(128);
 ...
 if (!pgres)
   {
    printf("Blad wykonania zapytania SQL, tablica uczen lub klasa nie istnieje?\n");
    printf("%d\n", __LINE__);
    return 1;
   }
 out = pgres2pc3(pgres, &a, &b);
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak uczniow lub klasa.\n");
    return 1;
   }
 for (i=0;i<a;i++) printf("klasa:%-10s\t ilosc uczniow: %s\n", out[i][1],out[i][2]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 return 0;
 ...
 *userCmd = (char*)malloc(128);
 ...
 printf("Ponizsza srednia jest srednia wszystkich ocen\n");
 printf("srednia szkoly: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 *userCmd = (char*)malloc(128);
 ...
 printf("Ponizsza srednia jest srednia wszystkich liczonych w sredniej ocen\n");
 printf("srednia szkoly: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 ...
 *userCmd = (char*)malloc(128);
 ...
 printf("ilosc uczniow w szkole: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 return 0;
 ...
 *userCmd = (char*)malloc(128);
 ...
 printf("ilosc wszystkich ocen w szkole: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 *userCmd = (char*)malloc(128);
 ...
 printf("ilosc ocen liczonych w sredniej w szkole: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 return 0;
...
int averageSchool(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak ocen nie mozna policzyc sredniej.\n");
    return 1;
   }
 free_p3c(&out,a,b);
 *userCmd = (char*)malloc(1024);
 ...
 if (!pgres)
   {
    printf("Blad wykonania zapytania SQL, tablica ocena lub przedmiot nie istnieje?\n");
    printf("%d\n", __LINE__);
    return 1;
   }
 clear_result(&pgres);
 ...
 *userCmd = (char*)malloc(128);
 ...
 if (!pgres)
   {
    printf("Blad wykonania zapytania SQL, widok srednie_klas nie istnieje?\n");
    printf("%d\n", __LINE__);
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak srednich nie mozna policzyc sredniej szkoly.\n");
    execute_query(connection,"DROP VIEW srednie_klas");
    execute_query(connection,"DROP VIEW srednie_uczniow");
    return 1;
   }
 printf("Ponizsza srednia jest srednia srednich klas.\n");
 printf("Liczonych jako srednie srednich przedmiotow.\n");
 printf("uwzgledniane sa tylko oceny liczone w szredniej.\n");
 printf("srednia szkoly: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 ...
 *userCmd = (char*)malloc(128);
 ...
 printf("Ponizsza srednia jest srednia srednich klas.\n");
 printf("Liczonych jako srednie srednich przedmiotow.\n");
 printf("Liczone sa takze oceny nie liczone w sredniej.\n");
 printf("srednia szkoly: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
...
int countMarksByClassOne(char** userCmd, PGconn* connection)
{
 ...
 free_p3c(&out,a,b);
 i=getDistinctClass(&idklasy,connection);
 if (i) { printf("Blad w wyborze klasy!\n"); return 1; }
 *userCmd = (char*)malloc(1024);
 sprintf(*userCmd,
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak klas lub ocen..\n");
    return 1;
   }
 printf("liczone sa wszystkie oceny.\n");
 for (i=0;i<a;i++) printf("klasa: %-8s, ilosc ocen: %s\n", out[i][1],out[i][2]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 *userCmd = (char*)malloc(1024);
 ...
 printf("tylko oceny liczone w sredniej.\n");
 for (i=0;i<a;i++) printf("klasa: %-8s, ilosc ocen: %s\n", out[i][1],out[i][2]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 ...
 *userCmd = (char*)malloc(1024);
 ...
 printf("liczone sa tylko oceny liczone w sredniej.\n");
 for (i=0;i<a;i++) printf("klasa: %-8s, ilosc ocen: %s\n", out[i][1],out[i][2]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 ...
 *userCmd = (char*)malloc(1024);
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak klas lub ocen, nie mozna policzyc sredniej.\n");
    return 1;
   }
 printf("Ponizsza srednia jest srednia wszystkich ocen uczniow danej klasy.\n");
 printf("brane pod uwage sa wszystkie oceny.\n");
 for (i=0;i<a;i++) printf("klasa: %-8s, srednia: %s\n", out[i][1],out[i][2]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 *userCmd = (char*)malloc(1024);
 ...
 *userCmd = (char*)malloc(512);
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak srednich nie mozna policzyc sredniej.\n");
    execute_query(connection,"DROP VIEW srednie_uczniow");
    return 1;
   }
 printf("Ponizsza srednia jest srednia srednich przedmiotow z ktorych oceny maja uczniowie tej klasy.\n");
 printf("brane pod uwage sa wszystkie przedmioty.\n");
 for (i=0;i<a;i++) printf("klasa: %-8s, srednia: %s\n", out[i][1],out[i][2]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 ...
 *userCmd = (char*)malloc(512);
 ...
 printf("brane pod uwage sa tylko oceny liczone w sredniej.\n");
 printf("Ponizsza srednia jest srednia srednich przedmiotow z ktorych oceny maja uczniowie tej klasy.\n");
 for (i=0;i<a;i++) printf("klasa: %-8s, srednia: %s\n", out[i][1],out[i][2]);
 free_p3c(&out,a,b);
 free(*userCmd);
 ...
 *userCmd=0;
 execute_query(connection,"DROP VIEW sredniel");
 printf("Ponizsza srednia jest srednia srednich klas liczonych jako\n");
 printf("srednia wszystkich ocen uczniow z danej klasy.\n");
 printf("brane pod uwage sa wszystkie oceny.\n");
 printf("srednia szkoly: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 *userCmd = (char*)malloc(1024);
 ...
 *userCmd=0;
 ...
 printf("Ponizsza srednia jest srednia srednich klas liczonych jako\n");
 printf("srednia wszystkich ocen uczniow z danej klasy.\n");
 printf("brane pod uwage sa oceny liczone w sredniej.\n");
 printf("srednia szkoly: %s\n", out[0][0]);
 free_p3c(&out,a,b);
 return 0;
}
 ...
 *userCmd = (char*)malloc(1024);
 ...
 printf("brane pod uwage wszystkie oceny.\n");
 for (i=0;i<a;i++) printf("%-18s %-15s %-8s, ocen: %s\n", out[i][1],out[i][2],out[i][3],out[i][4]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 *userCmd = (char*)malloc(1024);
 ...
 printf("brane pod uwage oceny liczone w sredniej.\n");
 for (i=0;i<a;i++) printf("%-18s %-15s %-8s, ocen: %s\n", out[i][1],out[i][2],out[i][3],out[i][4]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 ...
 *userCmd = (char*)malloc(1024);
 ...
 for (i=0;i<a;i++) printf("%-18s %-15s %-8s, srednia: %s\n", out[i][1],out[i][2],out[i][3],out[i][4]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 return 0;
 ...
 *userCmd = (char*)malloc(512);
 ...
 for (i=0;i<a;i++) printf("%-32s, ilosc ocen: %s\n", out[i][0],out[i][1]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 return 0;
 ...
 *userCmd = (char*)malloc(512);
 ...
 for (i=0;i<a;i++) printf("%-32s, srednia: %s\n", out[i][0],out[i][1]);
 free_p3c(&out,a,b);
 free(*userCmd);
 *userCmd=0;
 return 0;
...
int removeSubject(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak przedmiotow do usuniecia!\n");
    return 1;
   }
 printf("Oto zdefiniowane przedmioty:\n");
 for (i=0;i<a;i++) printf("%s, ",out[i][0]);
 printf("\n");
 free_p3c(&out,a,b);
 printf("Nazwa przedmiotu do usuniecia (lub jej jednoznaczna czesc): ");
 takestring(tmp);
 len = strlen(tmp);
 if (len>30) { printf("Nazwa jest dluzsza niz 30!\n"); return 1; }
 check = (char*)malloc(len+100);
 *userCmd = (char*)malloc(len+120);
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak takiego przedmiotu usuniecia!\n");
    return 1;
   }
 if (a>1)
   {
    free_p3c(&out,a,b);
    printf("Wybor niejednoznaczny, nastepujace przedmioty spelniaja wymagania:\n");
    for (i=0;i<a;i++) printf("%s, ", out[i][0]);
    ln();
    return 1;
   }
...
int modifySubject(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak przedmiotow do modyfikacji!\n");
    return 1;
   }
 printf("Oto zdefiniowane przedmioty:\n");
 for (i=0;i<a;i++) printf("%s, ",out[i][0]);
 printf("\n");
 free_p3c(&out,a,b);
 printf("Nazwa przedmiotu do modyfikacji (lub jego pierwsze litery): ");
 takestring(tmp);
 len = strlen(tmp);
 if (len>30) { printf("Nazwa jest dluzsza niz 30!\n"); return 1; }
 *userCmd = (char*)malloc(len+100);
 ...
 free_p3c(&out,a,b);
 strcpy(tmp,out[0][0]);
 printf("Nowa nazwa dla przedmiotu '%s': ", tmp);
 takestring(tmp2);
 len2 = strlen(tmp2);
 if (len2>30) { printf("Nowa nazwa jest dluzsza niz 30!\n"); return 1; }
 check = (char*)malloc(250);
 if (!check) { ERR printf("malloc failed!\n"); return 1; }
...
int modifyIndexSubject(char** userCmd, PGconn* connection)
{
 ...
 printf("\n");
 free_p3c(&out,a,b);
 printf("ID przedmiotu do modyfikacji: ");
 takestring(tmp);
 idprzedmiotu=-1;
 sscanf(tmp,"%d", &idprzedmiotu);
 if (idprzedmiotu<0) { printf("ID przedmiotu jest bledne\n"); return 1; }
 *userCmd = (char*)malloc(120);
...
int modifyIndexClass(char** userCmd, PGconn* connection)
{
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak klas do modyfikacji!\n");
    return 1;
   }
 ...
 printf("\n");
 free_p3c(&out,a,b);
 printf("ID klasy do modyfikacji: ");
 takestring(tmp);
 idklasy=-1;
 sscanf(tmp,"%d", &idklasy);
 if (idklasy<0) { printf("ID klasy jest bledne\n"); return 1; }
 *userCmd = (char*)malloc(120);
 check   = (char*)malloc(100);
 if (!*userCmd || !check) { ERR printf("malloc failed!\n"); return 1; }
 sprintf(check,"SELECT nazwa from klasa WHERE idklasy=%d", idklasy);
 ...
 free_p3c(&out,a,b);
 strcpy(tmp,out[0][0]);
 printf("Nowa nazwa dla klasy '%s': ", tmp);
 takestring(tmp2);
 len2 = strlen(tmp2);
 if (len2>7) { printf("Nowa nazwa jest dluzsza niz 7!\n"); return 1; }
...
int modifyClass(char** userCmd, PGconn* connection)
{
 ...
 printf("\n");
 free_p3c(&out,a,b);
 printf("Nazwa klasy do modyfikacji (lub jej pierwsze litery): ");
 takestring(tmp);
 len = strlen(tmp);
 if (len>7) { printf("Nazwa jest dluzsza niz 7!\n"); return 1; }
 *userCmd = (char*)malloc(len+100);
 ...
 if (a>1)
   {
    free_p3c(&out,a,b);
    printf("Wybor nie jest jednoznaczy, %d klas spelnia wymagania!\n",a);
    for (i=0;i<a;i++) printf("%s, ", out[i][0]);
    ln();
    return 1;
   }
...
int removeClass(char** userCmd, PGconn* connection)
{
 ...
 printf("\n");
 free_p3c(&out,a,b);
 printf("Nazwa klasy do usuniecia (lub czesc): ");
 takestring(tmp);
 len = strlen(tmp);
 if (len>7) { printf("Nazwa jest dluzsza niz 7!\n"); return 1; }
 *userCmd = (char*)malloc(len+100);
 ...
 if (!a)
   {
    free_p3c(&out,a,b);
    printf("Brak takiej klasy do usuniecia!\n");
    return 1;
   }
 if (a>1)
   {
    free_p3c(&out,a,b);
    printf("Wybor niejednoznaczny, nastepujace klasy spelniaja wymagania:\n");
    for (i=0;i<a;i++) printf("%s, ", out[i][0]);
    ln();
    return 1;
   }
...
void displayHelp()
{
 printf("DBklasa jest programem do zarzadzania ocenami klasy\n");
 printf("Potrzebna jest baza danyych PostgreSQL wersja >=7.4\n");
 printf("Program laczy sie z baza morgoth:morgoth, w razie\n");
 printf("Braku powyzszejk bazy nalezy wykonac: createdb morgoth\n");
 printf("A nastepnie: psql -f Morgoth.SQL\n");
 printf("Usuwanie klasy powoduje usuniecie jej uczniow (wszystkich)\n");
 printf("Usuniecie ucznia lub oceny powoduje usuniecie wszystkich\n");
 printf("Ocen z tego przedmiotu lub tego ucznia\n");
 printf("Wersja programu: alpha 0.01 (INITIAL):\n\t%s %s\n",__DATE__,__TIME__);
}
...
void saveDBToFile()
{
 char fname[1024];
 char syscmd[1500];
 printf("Podaj nazwe pliku (bez rozszerzenie SQL): ");
 takestring(fname);
...
int main(int lb, char** par)
{
 ...
 displayHelp();
 if (!connection) { printf("error connect to morgoth!\n"); return 2; }
 clear_result(&pgres);
 while (!done)
 {
  ...
  if (zn=='s')
    ...
    {
     printf("\nu\tusun tymczasowe tabele ...\n");
     printf("z\tzapisz baze do pliku ...\n");
     printf("\n\n>>>\t");
     z2 = mgetch();
  ...
  else if (zn=='p' || zn=='h' || zn=='?')
    {
     ...
     printf("s\twykonaj zapytanie SQL recznie\n");
     printf("d\tdodaj ...\n");
     printf("u\tusun ...\n");
     printf("z\tzobacz ...\n");
     printf("m\tmodyfikuj ...\n");
     printf("i\tmodyfikuj po indexach ...\n");
     printf("r\traporty ...\n");
     printf("w\tsposob wyszukiwania ...\n");
     printf("b\tbaza danych ...\n");
     printf("p,h,?\tpomoc\n");
     continue;
    }
  else if (zn=='w')
    {
     printf("\np\tdopasuj poczatek ...\n");
     printf("k\tdopasuj koniec ...\n");
     printf("s\tdopasuj srodek ...\n");
     printf("c\tdopasuj calosc ...\n");
     printf("\n\n>>>\t");
     z2 = mgetch();
     if (z2>='A' && z2<='Z') z2+=0x20;
     putchar('\n');
     ...
     if (z2=='k')
      {
       ...
       if (err) { printf("Blad w zmianie indexu klasy.\n"); continue; }
      }
     if (z2=='p')
      {
       err=modifyIndexSubject(&preparedStatement,connection);
       if (err) { printf("Blad w zmianie indexu przedmiotu.\n"); continue; }
      }
     if (z2=='u')
      {
       err=modifyIndexStudent(&preparedStatement,connection);
       if (err) { printf("Blad w zmianie indexu ucznia.\n"); continue; }
      }
    }
  else if (zn=='d')
    {
     ...
     printf("p\tprzedmiot ...\n");
     printf("u\tucznia ...\n");
     printf("c\tuczniow do klasy ...\n");
     printf("o\tocene ...\n");
     printf("r\tocene w/g przedmiotu ...\n");
     printf("z\tocene w/g ucznia ...\n");
     printf("l\tocene w/g klasy ...\n");
     printf("\n\n>>>\t");
     z2 = mgetch();
     if (z2>='A' && z2<='Z') z2+=0x20;
     putchar('\n');
     if (z2=='k')
       {
        ...
        if (err)
	  { printf("Blad przy dodawaniu klasy!\n"); continue; }
       }
     if (z2=='p')
       {
        err=addSubject(&preparedStatement,connection);
        if (err) { printf("Blad przy dodawaniu przedmiotu!\n"); continue; }
       }
     if (z2=='u')
       {
        err=addStudent(&preparedStatement,connection);
        if (err) { printf("Blad przy dodawaniu ucznia!\n"); continue; }
       }
     if (z2=='c')
       {
        err=addStudentsToClass(connection);
	if (!err) printf("Dodano wszystkich uczniow.\n");
	else printf("Blad przy dodawaniu uczniow.\n");
	continue;
       }
     if (z2=='o')
       {
        err=addMark(&preparedStatement,connection);
        if (err) { printf("Blad przy dodawaniu oceny.\n"); continue; }
       }
     if (z2=='r')
       {
        err=addMarkBySubject(&preparedStatement,connection);
	if (!err) printf("Dodano wszystkie oceny.\n");
	else printf("Blad przy dodawaniu ocen.\n");
        continue;
       }
     if (z2=='z')
       {
     ...
     if (z2=='k')
       {
        ...
        if (err) { printf("Blad przy modyfikacji klasy!\n"); continue; }
       }
     if (z2=='p')
       {
        err=modifySubject(&preparedStatement,connection);
        if (err) { printf("Blad przy modyfikacji przedmiotu!\n"); continue; }
       }
     if (z2=='u')
       {
        err=modifyStudent(&preparedStatement,connection);
        if (err) { printf("Blad przy modyfikacji studenta!\n"); continue; }
       }
     if (z2=='o')
       {
        err=modifyMark(&preparedStatement,connection);
        if (err) { printf("Blad przy modyfikacji oceny!\n"); continue; }
       }
    }
  else if (zn=='u')
    {
     ...
     if (z2=='k')
       {
        ...
        if (err) { printf("Blad przy usuwaniu klasy!\n"); continue; }
       }
     if (z2=='p')
       {
        err=removeSubject(&preparedStatement,connection);
        if (err) { printf("Blad przy usuwaniu przedmiotu!\n"); continue; }
       }
     if (z2=='u')
       {
        err=removeStudent(&preparedStatement,connection);
        if (err) { printf("Blad przy usuwaniu studenta!\n"); continue; }
       }
     if (z2=='o')
       {
        err=removeMark(&preparedStatement,connection);
        if (err) { printf("Blad przy usuwaniu oceny!\n"); continue; }
       }
    }
  else if (zn=='z')
    {
     ...
     printf("u\tucznia ...\n");
     printf("o\tocene ...\n");
     printf("r\tocene z przedmiotu...\n");
     printf("z\tocene ucznia ...\n");
     printf("w\toceny (wszystkie) ...\n");
     printf("\n\n>>>\t");
     z2 = mgetch();
     if (z2>='A' && z2<='Z') z2+=0x20;
     putchar('\n');
    ...
    {
     printf("\nk\tsrednie wszystkich klas (srednia przedmiotow klasy)...\n");
     printf("l\tsrednie wszystkich klas (srednia wstawionych ocen)...\n");
     printf("m\tsrednia wybranej klasy (srednia przedmiotow klasy)...\n");
     printf("n\tsrednia wybranej klasy (srednia wstawionych ocen)...\n");
     printf("w\tsrednia przedmiotow (ze wszystkich klas) ...\n");
     printf("p\tsrednia przedmiotow (w danej klasie) ...\n");
     printf("u\tsrednia uczniow (ze wszystkich klas)...\n");
     printf("z\tsrednia uczniow (w danej klasie)...\n");
     printf("s\tsrednia szkoly (srednia srednich klas 'k')...\n");
     printf("x\tsrednia szkoly (srednia srednich klas 'l')...\n");
     printf("v\tsrednia szkoly (srednia wszystkich ocen)...\n");
     printf("o\tilosc ocen w kazdej klasie ...\n");
     printf("q\tilosc ocen w wybranej klasie ...\n");
     printf("r\tilosc ocen z przedmiotow w kazdej klasie ...\n");
     printf("t\tilosc ocen z przedmiotow  w wybranej klasie ...\n");
     printf("d\tilosc uczniow w klasach\n");
     printf("e\tilosc uczniow w szkole\n");
     printf("a\tilosc ocen uczniow w klasach\n");
     printf("b\tilosc ocen uczniow w danej klasie...\n");
     printf("c\tilosc ocen w szkole ...\n");
     printf("\n\n>>>\t");
     z2 = mgetch();
     if (z2>='A' && z2<='Z') z2+=0x20;
     putchar('\n');
     if (z2=='k')
       {
        ...
        if (err) printf("Blad przy liczeniu sredniej!\n");
	continue;
       }
     if (z2=='l')
       {
     ...
     if (z2=='q')
       {
        ...
        if (err) printf("Blad przy liczeniu!\n");
	continue;
       }
     if (z2=='o')
       {
  ...
  else { printf("Nie znane polecenie...\n"); continue; }
  pgres = execute_query(connection, preparedStatement);
  if (!pgres)
   {
    printf("Blad wykonania zapytania SQL\n");
    printf("%d\n", __LINE__);
   }
  out = pgres2pc3(pgres, &a, &b);
