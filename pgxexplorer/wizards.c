...
#else
  gtk_signal_connect(GTK_OBJECT(entry), "changed", GTK_SIGNAL_FUNC(update_pidata),(gpointer)((INT64)i));
#endif
...
#else
  gtk_signal_connect(GTK_OBJECT(GTK_COMBO(combo)->entry), "changed", GTK_SIGNAL_FUNC(update_pidata_combo),(gpointer)((INT64)i));
#endif
...
void ctab_prop_combos(GtkWidget** combo, GtkWidget* table, INT I)
{
  ...
  *combo = gtk_combo_new();
  ...
  gtk_combo_set_popdown_strings(GTK_COMBO(*combo), list);
  g_list_free(list);
  gtk_table_attach(GTK_TABLE (table), *combo, 16, 24, I+2, I+3,GTK_FILL,GTK_FILL, 0, 0);
  //gtk_signal_connect(GTK_OBJECT(GTK_COMBO(combo)->entry), "changed", GTK_SIGNAL_FUNC(update_pidata_combo),(gpointer)(i));
  gtk_widget_show(*combo);
...
void ctab_type_combos(GtkWidget** combo, GtkWidget* table, INT i)
{
  ...
  *combo = gtk_combo_new();
  ...
  gtk_combo_set_popdown_strings(GTK_COMBO(*combo), list);
  g_list_free(list);
  gtk_table_attach(GTK_TABLE (table), *combo, 8, 16, i+2, i+3,GTK_FILL,GTK_FILL, 0, 0);
  //gtk_signal_connect(GTK_OBJECT(GTK_COMBO(combo)->entry), "changed", GTK_SIGNAL_FUNC(update_pidata_combo),(gpointer)(i));
  gtk_widget_show(*combo);
}

void update_title(GtkWidget* w, gpointer d)
{
 debug("%s:%d update_title()",__FILE__,__LINE__);
 strcpy((CHAR*)wdata.tabname,gtk_entry_get_text(GTK_ENTRY(w)));
 debug("tabname=%s\n", wdata.tabname);
}

void save_ctab(GtkWidget* w, gpointer d);


void setup_mainctrls(GtkWidget* table)
{
 GtkWidget **name, **type, **props, **anull, **def, *lname, *ltype, *lprops, *lanull, *ldef, *tab, *ltab, *sav;
 INT i;
 ...
 sav = gtk_button_new_with_label("Save Query");
 gtk_table_attach(GTK_TABLE(table), sav, 30, 40, 0, 1, GTK_FILL, GTK_FILL, 0, 0);
 gtk_widget_show(sav);
 gtk_signal_connect(GTK_OBJECT(sav), "clicked", GTK_SIGNAL_FUNC(save_ctab), NULL);
 ...
 ltab   = gtk_label_new("Table name:");
 gtk_table_attach(GTK_TABLE(table), ltab ,  0,  15, 0, 1, GTK_FILL, GTK_FILL, 0, 0);
 gtk_table_attach(GTK_TABLE(table), lname , 0,   8, 1, 2, GTK_FILL, GTK_FILL, 0, 0);
 gtk_table_attach(GTK_TABLE(table), ltype , 8,  16, 1, 2, GTK_FILL, GTK_FILL, 0, 0);
 gtk_table_attach(GTK_TABLE(table), lprops, 16, 24, 1, 2, GTK_FILL, GTK_FILL, 0, 0);
 gtk_table_attach(GTK_TABLE(table), lanull, 24, 32, 1, 2, GTK_FILL, GTK_FILL, 0, 0);
 gtk_table_attach(GTK_TABLE(table), ldef  , 32, 40, 1, 2, GTK_FILL, GTK_FILL, 0, 0);
 gtk_widget_show(lname);
 gtk_widget_show(ltype);
 gtk_widget_show(lprops);
 gtk_widget_show(lanull);
 gtk_widget_show(ldef);
 gtk_widget_show(ltab);
 tab = gtk_entry_new();
 gtk_entry_set_max_length(GTK_ENTRY(tab), STDSTRING-1);
 gtk_entry_set_editable(GTK_ENTRY(tab), TRUE);
 gtk_table_attach(GTK_TABLE(table), tab  , 15, 30, 0, 1, GTK_FILL|GTK_EXPAND|GTK_SHRINK, GTK_FILL, 0, 0);
 gtk_signal_connect(GTK_OBJECT(tab), "activate", GTK_SIGNAL_FUNC(update_title), NULL);
 gtk_signal_connect(GTK_OBJECT(tab), "changed",  GTK_SIGNAL_FUNC(update_title), NULL);
 gtk_widget_show(tab);
 for (i=0;i<wdata.ncols;i++)
  {
   ...
   gtk_table_attach(GTK_TABLE(table), name[i], 0, 8, i+2, i+3, GTK_FILL|GTK_EXPAND|GTK_SHRINK, GTK_FILL, 0, 0);
   ...
   gtk_table_attach(GTK_TABLE(table), anull[i], 24, 32, i+2, i+3, GTK_FILL, GTK_FILL, 0, 0);
   ...
   gtk_table_attach(GTK_TABLE(table), def[i], 32, 40, i+2, i+3, GTK_FILL, GTK_FILL, 0, 0);
...
int try_exec(char* errptr)
{
  CHAR*** ptr;
  INT row, col;
  debug("%s:%d try_exec()",__FILE__,__LINE__);
  if (!qptr) { sprintf(errptr, "Query is NULL"); return 0; }
  execute_printf(connection, &ptr, &row, &col, errptr, 0, qptr);
  if (row<0 || col<0) return 0; //if !ptr then ...
  free_p3c(&ptr, row, col);
  
  execute_printf(connection, &ptr, &row, &col, errptr, 0, "DROP TABLE %s", wdata.tabname);
  if (row<0 || col<0) return 0; //if !ptr then ...
  free_p3c(&ptr, row, col);
  return 1;
}

void generate_query_ctab()
{
 INT i;
 char tmps[STDSTRING+64];
 char tmps2[STDSTRING+64];
 debug("%s:%d generate_query_ctab()",__FILE__,__LINE__);
 if (qptr) free(qptr);
 qptr = NULL;
 qptr = malloc(wdata.ncols*(STDSTRING+64)+MAX_QUERY_LENGTH);
 if (!qptr) fatal_here(HERE, "malloc failed!");
 if (!strcmp(wdata.tabname, "")) 
   {
    gtk_dialog_printf("Set table name!");
    free(qptr);
    qptr = NULL;
    return ;
   }
 sprintf(qptr, "CREATE TABLE %s(", wdata.tabname);
 for (i=0;i<wdata.ncols;i++)
   {
    sprintf(tmps, "%s %s", gtk_entry_get_text(GTK_ENTRY(wdata.name[i])), 
	    gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(wdata.type[i])->entry)));
    if (!(GTK_TOGGLE_BUTTON(wdata.anull[i])->active))
      {
       strcat(tmps, " NOT NULL");
      }
    if (strcmp(gtk_entry_get_text(GTK_ENTRY(wdata.def[i])),""))
      {
       if (numeric(gtk_entry_get_text(GTK_ENTRY(wdata.def[i]))))
       sprintf(tmps2," DEFAULT %s", gtk_entry_get_text(GTK_ENTRY(wdata.def[i])));
       else 
       sprintf(tmps2," DEFAULT '%s'", gtk_entry_get_text(GTK_ENTRY(wdata.def[i])));
       strcat(tmps, tmps2);
      }
    if (strcmp(gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(wdata.props[i])->entry)), "-other properties-"))
      {
       if (!strcmp(gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(wdata.props[i])->entry)), "primary key"))
	   strcat(tmps, " PRIMARY KEY");
       else
         {
          sprintf(tmps2, " %s", gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(wdata.props[i])->entry)));
	  strcat(tmps, tmps2);
         }
      }
    if (i < (wdata.ncols-1)) strcat(tmps,", ");
    else strcat(tmps, ")");
    strcat(qptr, tmps);
   }
 debug("qptr = %s\n", qptr);
...
void save_ctab(GtkWidget* w, gpointer d)
{
 debug("%s:%d save_ctab()",__FILE__,__LINE__);
 generate_query_ctab();
 CHAR *filen;
 ...
 if (f)
   {
    fclose(f);
    if (!gtk_dialog_yes_no_printf("Overwite file: %s?", filename)) return;
   }
 f = fopen(filename,"w");
 if (!f)
   {
...
void setup_txtscene(GtkWidget* table)
{
 ...
 debug("%s:%d setup_txtscene()",__FILE__,__LINE__);
 ...
 switch (wizstep)
 {
  ...
  case 2:	/* 2 -->  1 */
	    if (ieditor)
	      ...
	      {
	       INT x;
               gtk_widget_destroy(wdata.sav);
               gtk_widget_destroy(wdata.tab);
               gtk_widget_destroy(wdata.ltab);
 ...
 switch (wizstep)
   {
    ...
    case 2:	/* currently ready for load/save options */
	    ...
	    INT x;
	    CHAR ans[MAX_QUERY_LENGTH*20+100];
	    generate_query_ctab();
	    if (!try_exec(ans))
	      {
		ans[191] = 0;		//may be too long!
  		if (gtk_dialog_yes_no_printf_big("DB error:\n%s\nClose wizard?", ans))
		  {
		   free(qptr);
   	           qptr = NULL;
	           gtk_main_quit();
		...
		else { free(qptr); qptr = NULL; break; }
	      }
            gtk_widget_destroy(wdata.sav);
            gtk_widget_destroy(wdata.tab);
            gtk_widget_destroy(wdata.ltab);
...
void init_wdata()
{
 ...
 wdata.tab   = wdata.ltab   = wdata.sav    = NULL;
}

void clean_vars()
{
 debug("%s:%d clean_vars()", __FILE__,__LINE__);
 qptr = NULL;
}
...
CHAR* ctable_wizard()
{
 ...
 //FIXME if freed but not NULLed ?? Fucked up things..
 //I hope I removed that possibilities
 if (qptr) free(qptr);
 qptr = NULL;
 wizstep = 0;
 init_wdata();
 ...
 window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
 gtk_widget_set_usize(GTK_WIDGET(window), 780, 450);
 gtk_container_set_border_width(GTK_CONTAINER(window), 5);
